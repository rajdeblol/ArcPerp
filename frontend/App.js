...
function AppBody() {
    ...
    const marketStatePda = useMemo(() => PublicKey.findProgramAddressSync([Buffer.from("market-state")], PROGRAM_ID)[0], []);
    const traderAccountPda = useMemo(() => {
        if (!wallet.publicKey) {
            return null;
        }
        return PublicKey.findProgramAddressSync([Buffer.from("trader"), marketStatePda.toBuffer(), wallet.publicKey.toBuffer()], PROGRAM_ID)[0];
